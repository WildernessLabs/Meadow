...
int client_cert_initialize() {
    ...
    syslog(LOG_INFO, "client_cert_len: %zu\n", client_cert_len);
    syslog(LOG_INFO, "private_key_len: %zu\n", private_key_len);
    ...
    int client_cert_store_ret = client_cert_store_credentials(
        ...
        (const char *)private_key, private_key_len + 1, NULL
    );
    ...
    syslog(LOG_INFO, "client_cert_retrieved_len ret: %s\n", client_cert_retrieved);
    syslog(LOG_INFO, "private_key_retrieved_len ret: %s\n", private_key_retrieved);
